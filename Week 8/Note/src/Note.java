 ...
 * Note Class
 */
public class Note implements Comparable<Note> { // implement Comparable interface
    private Map<Integer,String> piano = new LinkedHashMap<>(); // declare Map to hold LinkedHashMap values
    ...
    private double frequency, bpm, noteLength; // frequency of note $ beats per minute
    ...
    public double getNoteLength() { return noteLength; }
    public double getFreq() { return frequency; } // @return frequency formatted to 2 decimal places
    ...
    public int compareTo(Note n) {
        if (this.frequency > n.getFreq()) // if this note is greater than compared note
            return 1; // return 1
        else if (this.frequency < n.getFreq()) // if this note is less than compared note
            return -1; // return -1
    ...
    public List<Note> sortNotes(List<Note> notes){
        Collections.sort(notes, Comparator.comparing(Note::getNoteLength).thenComparing(Note::getFreq)); // sort list using Collections class and Comparator interface
        return notes;
    }
}