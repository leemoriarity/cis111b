...
import static org.apache.commons.math3.complex.Complex.NaN;
...
public class Driver {
    ...
    public void submit() throws IOException {
        ...
        String s = "";
        if (sol.length == 1) {
            s += "x = " + cf.format(sol[0].getReal());
        }
        if (sol.length == 2 && !sol[0].equals(NaN)) {
            if (cf.format(sol[0]).contains("i")) {
                s += "x₁ = " + cf.format(sol[0]) + "\n";
                s += "x₂ = " + cf.format(sol[1]) + "\n\n";
                s += "Sum of roots: " + cf.format(quad.sumOfRoots()) + "\n";
                s += "Product of roots: " + cf.format(quad.prodOfRoots());
            } else {
                s += "x₁ = " + cf.format(sol[0].getReal()) + "\n";
                s += "x₂ = " + cf.format(sol[1].getReal()) + "\n\n";
                s += "Sum of roots: " + cf.format(quad.sumOfRoots()) + "\n";
                s += "Product of roots: " + cf.format(quad.prodOfRoots());
            }
        }
        if (sol[0].equals(NaN)) {
            if (Double.parseDouble(textA.getText()) == 0 && Double.parseDouble(textB.getText()) != 0 && Double.parseDouble(textC.getText()) == 0) // if a and c are 0
                s = "x = 0";
            if (Double.parseDouble(textA.getText()) == 0 && Double.parseDouble(textB.getText()) == 0 && Double.parseDouble(textC.getText()) == 0) // if a, b, and c are all 0
                s = "x = all values";
            if (Double.parseDouble(textA.getText()) == 0 && Double.parseDouble(textB.getText()) == 0 && Double.parseDouble(textC.getText()) != 0) // if a and b are 0
                s = "No solution. " + textC.getText() + " ≠ 0";
        }
        textSol.setText(s);
        ...
        if (quad.getA() == 0 && quad.getB() == 0 && quad.getC() != 0 || quad.getA() == 0 && quad.getB() != 0 && quad.getC() == 0 || quad.getA() == 0 && quad.getB() != 0 && quad.getC() != 0)
